...
export default function Gallery({ eventos, refreshQueue, query }) {
  ...
  function getNextPage() {
    refreshQueue(
      `https://api.seatgeek.com/2/events?per_page=10&page=2&q=${query}&client_id=MTQyMjc2OTd8MTcwOTEwNTkyNi4xODg0MjU1`,
    );
  ...
  if (eventos.length > 0) {
    ...
    // Slice the corresponding eventos and create Cards for them
  ...
  const btnClasses =
    "text-white border-2 border-gray-700 rounded-lg p-1 mx-2 font-semibold hover:bg-gray-700 hover:opacity-70 transition-all duration-200 ease-in-out";
